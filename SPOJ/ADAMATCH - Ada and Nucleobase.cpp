#include <bits/stdc++.h>
 
#define endl '\n'
#define lli long long int
#define ld long double
#define forn(i,n) for (int i = 0; i < n; i++)
#define all(v) v.begin(), v.end()
#define fastIO(); ios_base::sync_with_stdio(0); cin.tie(0); cout.tie(0);
#define SZ(s) int(s.size())

using namespace std;

typedef vector<lli> VLL;
typedef vector<int> VI;

lli powerMod(lli a, lli b, lli mod){
	lli res = 1;
	b %= mod-1;
	if(b < 0) b += mod-1;
	while(b){
		if(b & 1) res = (res * a) % mod;
		b >>= 1;
		a = (a * a) % mod;
	}
	return res;
}

int nearestPowerTwo(int n)
{
    int r = 1;
    while(r < n) r<<=1;
    return r;
}

template<int p, int g, typename T>
void ntt(vector<T> & A, bool inv){ //O(n log n)
	int n = SZ(A);
	for(int i = 1, j = 0; i < n - 1; ++i){
		for(int k = n >> 1; (j ^= k) < k; k >>= 1);
		if(i < j) swap(A[i], A[j]);
	}

	vector<lli> wp(n>>1, 1);

	for(int k = 1; k < n; k <<= 1){
		lli wk = powerMod(g, (inv?-1:1) * (p - 1) / (k<<1), p);
		for(int j = 1; j < k; ++j)
			wp[j] = (wp[j - 1] * wk) % p;

		for(int i = 0; i < n; i += k << 1){
			for(int j = 0; j < k; ++j){
				lli u = A[i + j], v = (A[i+j+k] * wp[j]) % p;
				A[i+j] = (u + v < p) ? u + v : u + v - p;
				A[i+j+k] = (u - v >= 0) ? u - v: u - v + p;
			}
		}
	}

	if(inv){
		lli inv_n = powerMod(n, p - 2, p);
		for(auto &x: A) x = (x*inv_n)%p;
	}
}
 
template<int p, int g, typename T>
vector<T> multiply(vector<T> A, vector<T> B){ //O(n logn) it uses NTT
	int sz = SZ(A) + SZ(B) - 1;
	int size = nearestPowerTwo(sz);
    
	A.resize(size), B.resize(size);
	ntt<p, g>(A, false), ntt<p, g>(B, false);
	for(int i = 0; i < size; i++)
		A[i] = (lli)A[i] * B[i] % p;
	ntt<p, g>(A, true);

	A.resize(sz);
	return A;
}

const int p = 7340033, g = 5;

int main () {
	fastIO();
    string s, t; cin>>s>>t;
    int n = s.length(), m = t.length();
    vector<int> sa(n), sc(n), st(n), sg(n);
    vector<int> ta(m), tc(m), tt(m), tg(m);

    for(int i = 0; i<n; i++)
    {
        char c = s[i];
        if(c == 'A') sa[i] = 1;
        else if(c == 'C') sc[i] = 1;
        else if(c == 'T') st[i] = 1;
        else sg[i] = 1;
    }

    for(int i = 0; i<m; i++)
    {
        char c = t[i];
        if(c == 'A') ta[i] = 1;
        else if(c == 'C') tc[i] = 1;
        else if(c == 'T') tt[i] = 1;
        else tg[i] = 1;
    }

    reverse(all(ta));
    reverse(all(tc));
    reverse(all(tt));
    reverse(all(tg));

    auto Ca = multiply<p,g>(sa, ta);
    auto Cc = multiply<p,g>(sc, tc);
    auto Ct = multiply<p,g>(st, tt);
    auto Cg = multiply<p,g>(sg, tg);

    int ans = 1e9;
    for(int i = m-1; i<=SZ(Ca)-m; i++)
    {
        int total = Ca[i] + Cc[i] + Ct[i] + Cg[i];
        ans = min(ans, m - total);
        //cerr << "Con " << i - (m-1) << " shifts, hay : " << total << endl;
    }
    cout << ans << endl;


	return 0;
}