...
template<int p = 7340033, int g = 3, typename T>
...
const int maxN = 1e5;
const int MOD = 7340033; // Initialize
...
int main () {
    ...
    lli n, a, b, c; cin>>n>>a>>b>>c;


    Poly A(2*a + 1);
    for(int i = 0; i <= a; i++) {
        int mult = ((a-i) % 2 == 0 ? 1 : MOD-1);
        A[2*i] = nCr(a, i) * mult % MOD;
    }
    ...
    for(int i = 0; i <= 2*(a+b); i++) {
        ans += P[i] * powerMod(i + c - a - b, n, MOD) % MOD;
        if(ans >= MOD) ans -= MOD;
    }

    lli inv_2 = powerMod(2, MOD-2, MOD) % MOD;
    cout << ans * powerMod(inv_2, a+b, MOD) % MOD << endl;
