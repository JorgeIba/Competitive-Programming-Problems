...
struct TreapNode{
	...
	TreapNode(T value = 0): value(value), key(dis(rng)), size(1), left(NULL), right(NULL), sum(value), add(0), rev(false){}
};
...
int main()
{
    ...
    while(m--)
    {
        ...
        {
            lli aux = x+1;
            cout<< T->lessThan(aux)<<endl;
            //cout<<LEQ(root, x)<<endl;
        }
        else
        {
            x--;
