...
struct SegmentTreeBeats {
    ...
    void updateNodeMIN(int curr, T value){
        ...
        ST[curr].lazyMIN = min(ST[curr].lazyMIN, value);
    }
...
int main () {
    ...
    for(int i = 0; i < q; i++)
    {
        lli t, ql, qr; cin>>t>>ql>>qr;
        if(t == 1)
        {
            lli x; cin>>x;
            beats->updateMIN(ql, qr, x);
        } else {
            cout << beats->query_sum(ql, qr) << endl;
        }

        // beats->debug_tree(1, 1, n);
    }
