...
int128 f(lli p, lli a){
    int128 aux = (pa*pa - 1) * powerMod(p+1, MOD-2, MOD);
...
int main()
{
    ...
    lli t; cin>>t;
    vector<lli> res(t);
    lli inverse2 = powerMod(2,MOD-2,MOD);
    forn(i,0,t)
    {
        //lli n; cin>>n;
        lli n = aleatorio(2, 1e18);
        int128 F=1, G=1;
        if(n==1)
        {
            res[i] = 1;
        ...
        for(auto x: fact)
        {
            pa =  powerMod(x.first, x.second, MOD);
            F = (F * f(x.first, x.second)) %MOD;
            G = (G* g(x.first, x.second)) % MOD;
        }
        lli ans = (((F + G)%MOD)*inverse2)%MOD;
        res[i] = ans;
    }
    for(auto x: res)
    {
