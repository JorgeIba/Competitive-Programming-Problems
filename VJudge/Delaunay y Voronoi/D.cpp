...
int main () {
    ...
    forn(i, n){
        ...
        for(auto p: region){
            if(pointInPolygon(poly, p) == 0) continue;
            ans = max(ans, (p - tran).length());
        }
        ...
        for(int i = 0; i < m; i++){
            for(int j = 0; j < n; j++){
                if(intersectSegmentsInfo(region[i], region[(i+1)%m], poly[j], poly[(j+1)%n]) == 1){
                    auto intersection = intersectLines(region[i], region[(i+1)%m] - region[i], poly[j], poly[(j+1)%n] - poly[j]);

                    ans = max(ans, (intersection - tran).length());
                }
            }
        }
    }
